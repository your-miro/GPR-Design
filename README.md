# GPR-Design 
## (This project has been deprecated, and the following results are only valid for signals short in time)
Moreover, the aim of the project was to try to go for a frequency-modulated continuous wave (FMCW) solution rather than the typical pulsed radar when making a GPR.
```
Design1.pll analysed at Sun May 18 20:48:14 2025 
 
PLL Chip is ADF4159 
Notes:  
VCO is 100176SM10 
Reference is custom 
 
Frequency Domain Analysis of PLL 
  Analysis at PLL output frequency of 1.0GHz 
 
Phase Noise Table  
Freq	Total	VCO	Ref	Chip	SDM	Filter 
 100 	-99.72 	-167.8 	--	-99.72 	-256.8 	-149.8  
1.00k	-107.8 	-147.8 	--	-107.8 	-216.8 	-137.7  
10.0k	-110.0 	-127.8 	--	-110.8 	-176.4 	-118.1  
 100k	-106.8 	-117.3 	--	-112.1 	-137.2 	-109.0  
1.00M	-134.3 	-140.9 	--	-150.9 	-135.9 	-147.1  
 
Reference Spurious 
  Noise and Jitter Calculations include the first 10 ref spurs 
  First three spurs:  -162 dBc   -184 dBc   -198 dBc 
 
Fractional-N Spur Estimate (worst case) 
No significant spurs 
 
Phase jitter using brick wall filter  
  from 1.00kHz to 20.0MHz 
  Phase Jitter  449fs rms
 
ACP - Channel 1 
  Channel 1 is centred 25.0kHz from carrier with bandwidth 15.0kHz 
  Power in channel =  -65.0dBc
 
       ----    End of Frequency Domain Results   ---- 
 
Transient Analysis of PLL 
  Modulation transient analysis  
       ----    End of Time Domain Results   ---- 
```
![image](https://github.com/user-attachments/assets/40c20554-4f74-4dbd-95c6-ecc16c0e8186)

![image](https://github.com/user-attachments/assets/9e7a7255-bb53-4373-9b56-5e874dce3fe3)

![image](https://github.com/user-attachments/assets/5c82837f-3934-4e46-beef-87ad0ea788ab)

